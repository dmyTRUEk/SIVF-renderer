'''
This file contains all SIVF KeyWords (KW)

In fact, this files determines keywords which is used in '*.sivf' file

P.S. all is sorted:
1) by scope
2) by alphabet order
'''



# GLOBAL CONST:
KW_TRUE = 'true'
KW_FALSE = 'false'

# ROOT PROPERTIES:
KW_CANVAS_WH = 'canvas_wh'
KW_COLOR_SCHEME = 'color_scheme'
KW_IMAGE = 'image'
KW_VARS = 'vars'

# BLENDING AND BLENDING TYPES:
KW_BLENDING = 'blending'
KW_BLENDINGTYPE_ADD = 'add'
KW_BLENDINGTYPE_AVG = 'avg'
KW_BLENDINGTYPE_DEFAULT = 'default'
KW_BLENDINGTYPE_MIN = 'min'
KW_BLENDINGTYPE_MAX = 'max'
KW_BLENDINGTYPE_OVERLAP = 'overlap'

# GLOBAL SHAPE PROPERTIES:
KW_COLOR = 'color'
KW_INVERSE = 'inverse'
KW_INVERSE_DEFAULT = False
KW_USED = 'used'
KW_USED_DEFAULT = True
KW_XY = 'xy'

# ENTITIES AND THEIR CUSTOM PROPERTIES:
KW_CIRCLE = 'circle'
KW_CIRCLE_R = 'r'

KW_COMBINE = 'combine'
KW_COMBINE_FIGURES = 'figures'
KW_COMBINE_TYPE = 'type'
KW_COMBINE_TYPE_1PLUS2 = '1+2'
KW_COMBINE_TYPE_2PLUS1 = '2+1'
KW_COMBINE_TYPE_1MINUS2 = '1-2'
KW_COMBINE_TYPE_2MINUS1 = '2-1'
KW_COMBINE_TYPE_1PRODUCT2 = '1*2'
KW_COMBINE_TYPE_2PRODUCT1 = '2*1'
KW_COMBINE_TYPE_1SYMDIFF2 = '1%2'    # SymDiff = Symmetric Difference
KW_COMBINE_TYPE_2SYMDIFF1 = '2%1'

KW_GRADIENT = 'gradient'
KW_GRADIENT_FADING = 'fading'
KW_GRADIENT_FADING_DEFAULT = True
KW_GRADIENT_POINTS = 'points'
KW_GRADIENT_SIGMA = 'sigma'

KW_LAYER = 'layer'
KW_LAYER_DELTA_XY = 'delta_xy'

KW_MESH = 'mesh'
KW_MESH_N_XLEFT_YDOWN_XRIGHT_YUP = 'n_xleft_ydown_xright_yup'

KW_SQUARE = 'square'
KW_SQUARE_SIDE = 'side'

KW_TRIANGLE = 'triangle'



